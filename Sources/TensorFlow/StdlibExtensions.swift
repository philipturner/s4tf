...
struct Quensor: Differentiable & AdditiveArithmetic {
...
func Tensor_stack(_ lhs: Quensor) -> Quensor {
  Tensor_init(stacking: [lhs])
}


@differentiable(reverse)
func Tensor_init(stacking tensors: [Quensor]) -> Quensor {
  fatalError()
}

@derivative(of: Tensor_init(stacking:))
 func _vjpStacking(
  stacking tensors: [Quensor]
) -> (value: Quensor, pullback: (Quensor) -> Array<Quensor>.TangentVector) {
  fatalError()
}
